...
use image::{GenericImageView, GrayImage, imageops, ImageReader, Luma};
use linfa::Dataset;
...
impl BackgroundSubtractor {
    ...
    pub(crate) fn new(initial_frame: &GrayImage) -> Self {
        ...
        let bg = Array2::from_shape_fn((height as usize, width as usize), |(y, x)| {
            initial_frame.get_pixel(x as u32, y as u32).0[0] as f32
        });
    ...
    pub(crate) fn apply(&mut self, frame: &GrayImage) -> GrayImage {
        ...
        for y in 0..height {
            for x in 0..width {
                let pixel_value = frame.get_pixel(x, y).0[0] as f32;
                let bg_value = self.background[[y as usize, x as usize]];

                // Compute the absolute difference between the two grayscale pixels
                let diff = (pixel_value - bg_value).abs() as u8;

                // Apply a weighted average of the pixel and background value to ensure things like gradual light change don't affect our detection
                self.background[[y as usize, x as usize]] = (ALPHA * pixel_value) + ((1.0 - ALPHA) * bg_value);

                // Apply a threshold to the differences and compute our difference 'array' / 'image'
                output.put_pixel(x, y, Luma([if diff > THRESHOLD { 255 } else { 0 }]));
            }
        }
        ...
        if let Some(latest_frame) = binding.as_ref() {
            ...
            if self.last_detection.is_none() ||
                ...
                if self.motion.is_none() {
                    ...
                    let mut total_amt_of_points = 0;

                    // We accumulate all changed points into a vector and keep track of the total amount
                    let mut data_vec = Vec::with_capacity(total_amt_of_points * 2);
                    let mut targets: Vec<f64> = Vec::new();
                    for (x, y, pixel) in diff_result.enumerate_pixels() {
                        if pixel[0] == 255 {
                            data_vec.push(x as f64);
                            data_vec.push(y as f64);

                            targets.push(1f64);
                            total_amt_of_points += 1;
                        }
                    }
