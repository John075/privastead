...
fn get_input_camera_secret() -> Vec<u8> {
    ...
    let file = File::open(pathname).expect("Could not open file");
    let mut reader =
...
fn create_camera_groups(
    ...
    let app_fcm_key_packages = pair_with_app(&mut stream, client_fcm.key_packages(), secret);
    create_group_and_invite(
        &mut stream,
        camera,
        client_fcm,
...
fn core(
    ...
    File::create(state_dir.clone() + "/registration_done").expect("Could not create file");
